...
export default function AdminVendorVerify() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/admin/vendors?status=${tab}`, {
        headers: {
  ...
  const viewAttachments = async (applicant_id) => {
    try {
      const res = await axios.get(`${API}/vendor/application/${applicant_id}`, {
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border border-gray-700">
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.applicant_id} className="border-t border-gray-700 text-gray-900">
                  <td className="p-2">{i + 1}</td>
                  <td className="cursor-pointer ">
                    <button onClick={() => viewAttachments(r.applicant_id)}>
                      {r.applicant_id}
                    </button>
                  </td>
                  <td>{r.username}</td>
                  ...
                  <td className="flex flex-wrap gap-2 p-2">
                    ...
                    {tab === "PENDING" && (
                      <>
                        <button
                          onClick={() => updateStatus(r.applicant_id, "REJECT")}
                        ...
                        <button
                          onClick={() => updateStatus(r.applicant_id, "VERIFY")}
                    ...
                    {(tab === "APPROVED" || tab === "PENDING") && (
                      <button
                        onClick={() => {
                          if (window.confirm("Are you sure you want to permanently delete this vendor and all associated details? This action cannot be undone.")) {
                            updateStatus(r.applicant_id, "DELETE_VENDOR");
                          }
                        }}
                        className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded font-medium transition-colors"
                      >
                        Delete Vendor
                      </button>
                    )}
          ...
          <div className="bg-gray-900 p-6 rounded-lg w-full max-w-5xl mx-auto my-10 space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <Info
                  label="Business Name"
                  value={attachments.ref_name || attachments.name}
                />
                ...
                <Info
                  label="User Discount (%)"
                  value={attachments.user_discount}
                />
                <Info
                  label="Company Discount (%)"
                  value={attachments.company_discount}
                />
                <Info
                  label="Vendor Offer (%)"
                  value={attachments.vendor_offer_user}
                />
                <Info
                  label="Company Offer (%)"
                  value={attachments.company_offer_user}
                />

                <Info
                  label="Offer Start Date"
                  value={attachments.offer_start_date}
                />
                <Info
                  label="Offer End Date"
                  value={attachments.offer_end_date}
                />
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <Info
                  ...
                  value={attachments.personal_address}
                  full
                />
              </div>
            </section>
          ...
          <div className="bg-gray-900 p-6 rounded-lg w-full max-w-5xl mx-auto my-10 space-y-6">
            ...
            <section>
              ...
              <AttachmentLink
                ...
                path={attachmentImage.personal_pan}
              />
              <AttachmentLink
                label="Aadhaar Card"
                path={attachmentImage.aadhaar_card}
              />
            </section>
